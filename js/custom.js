...
/*global $, console, alert, Element, FileReader*/
...
for (loopImgs = 0; loopImgs < 51; loopImgs++) {
    if (loopImgs <= 9) {
        imgs[loopImgs] = '0' + loopImgs + '.JPG';
    } else {
        imgs[loopImgs] = loopImgs + '.JPG';
    }
}
imgs.shift();
/*****************************************************************************************/
...
function setTimeDownOnchangeLevel() {
    ...
    case 'Easy':
        timeDown = 1.5 * 60 * 10; // 03-08-2020
        break;
    case 'Middle':
        timeDown = 4 * 60 * 10; // 03-08-2020
        break;
    case 'Hard':
        timeDown = 8 * 60 * 10; // 03-08-2020
        break;
    case 'Very Hard':
        timeDown = 12 * 60 * 10; // 03-08-2020
        break;
    }
...
function addRandomImgs(numArr) {
    ...
    for (x = 0; x < numArr * 2; x = x + 1) {
        boxs[x].children[1].children[0].setAttribute('src', 'imgs/imgsRandom/' + newarrImgs[x]);
    }
}
/*****************************************************************************************/
...
/*****************************************************************************************/
function viewPhoto($this) { // input file [this]
    'use strict';
...
wraViewPhotoUser.onmouseover = function () {
    'use strict';
    if (viewPhotoUser.getAttribute('src') === 'imgs/user.png') {
...
removePhotoUser.onclick = function () {
    'use strict';
    localStorage.setItem('userphoto', 'imgs/user.png');
...
function setUserPhoto() {
    'use strict';
    var newPhoto = localStorage.getItem('userphoto') || 'imgs/user.png';
...
function funcLoopChangeLevel(numBoxs, numDelayGoker) {
    ...
    var newFuncAddVisibile = function () { visibileImg(this, numBoxs, numDelayGoker); },
        imgGoker = document.querySelectorAll('img[src="imgs/imgsRandom/20.JPG"]'),
        gok;
    for (x = 0; x < boxs.length; x++) {
...
/*****************************************************************************************/
// Start Function Prevent Show Default Context Menu
